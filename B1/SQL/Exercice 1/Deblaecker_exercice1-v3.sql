...
SELECT e.emp_no, first_name, last_name, hire_date, SUM(salary) AS sommeSalaire FROM employees e, salaries s WHERE e.emp_no=s.emp_no AND first_name='Richard' GROUP BY salary HAVING SUM(salary)>'1000000' ORDER BY last_name;
-- Rqt 22
...
-- Alternative 1 :
SELECT employees.emp_no, first_name, last_name, salary AS maxSalary FROM employees INNER JOIN salaries ON employees.emp_no=salaries.emp_no WHERE employees.emp_no=(SELECT emp_no FROM salaries ORDER BY salary DESC LIMIT 1) ORDER BY salary DESC LIMIT 1;
-- Alternative 2
